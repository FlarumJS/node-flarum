...
function routes (app, passport) {
	...
	app.get('/auth/google/callback', function (req, res, next) {
		passport.authenticate('google', {
			successRedirect: req.flash('authRedirectUrl') || forumPath,
			failureRedirect: req.flash('authRedirectUrl') && req.flash('authRedirectUrl') + '?loginModal=true&authError=true' || forumPath + '?loginModal=true&authError=true'
		}, function (err, user, info) {
			if (err == 'TokenError: Code was already redeemed.') {
				err.status = 403;
				err.message = 'Google Auth - Code was already redeemed<br/><small><a href=\"' + forumPath + 'auth/google\">Click to go back and try again</span></small></a>';
				return next(err);
			}
			if (!user || user.length == 0) return res.redirect('/login');

			debug('User logged in with Google:');
			debug(user);

			req.logIn(user, function (err) {
				if (err) {
					err.status = 500;
					err.message = 'Google Auth - Problem while signing you in<br/><small><a href=\"' + forumPath + 'auth/google\">Click to go back and try again</span></small></a>';
					return next(err);
				}
				return res.redirect('/');
			})
