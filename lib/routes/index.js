...
function routes (app, passport) {
	...
	app.get('/api/discussions', function (req, res, next) {
		var tag = (req.query && req.query.tag && req.query.tag != null) && req.query.tag || undefined;
		var tagRegExp = tag && new RegExp(tag, 'i') || new RegExp('().*', 'i');
		var search = (req.query && req.query.q && req.query.search != null) && req.query.q || undefined;
	...
	app.post('/flarum/install', function (req, res) {
		...
		if (!forumTitle) return res.status(403).send('Forum Title can\'t be empty');

		if (!mongodb.host) return res.status(403).send('MongoDB Host can\'t be empty');
		if (!mongodb.database) return res.status(403).send('MongoDB Database can\'t be empty');
		if (mongodb.username && !mongodb.password) return res.status(403).send('You forgot to type the MongoDB Password!');
		if (!mongodb.username && mongodb.password) return res.status(403).send('You forgot to type the MongoDB Username!');

		if (!admin.username) return res.status(403).send('Admin Username can\'t be empty');
		if (admin.username.length < 4) return res.status(403).send('Username is too short (min 4 char)');
		if (!admin.email) return res.status(403).send('Admin Email can\'t be empty');
		if (!admin.password) return res.status(403).send('Admin Password can\'t be empty');
		if (admin.password.length < 6) return res.status(403).send('Password is too short (min 6 char)');
		if (!admin.passwordConfirmation) return res.status(403).send('Confirm Password can\'t be empty');
		if (admin.password != admin.passwordConfirmation) return res.status(403).send('Passwords do not match');
